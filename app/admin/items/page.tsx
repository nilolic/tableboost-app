...
type Item = {
  ...
  categoryId:string; category:{name:string}; imageUrl?:string|null;
  available:boolean; isBoosted:boolean; boostLevel:number; order?:number;
};
...
export default function ItemsPage(){
  ...
  const [newItem,setNewItem]=useState<any>({
    ...
    isBoosted:false, boostLevel:0, available:true
  });

  const load=async()=>{
    const cRes=await fetch("/api/admin/categories");
    const iRes=await fetch("/api/admin/menu");
    const cData=await cRes.json();
    const iData=await iRes.json();
    setCats(cData.categories||[]);
    setItems(iData.items||[]);
    if(cData.categories?.[0] &&!newItem.categoryId){
      setNewItem((p:any)=>({...p, categoryId:cData.categories[0].id }));
    }
  };
  ...
  const create=async()=>{
    if(!newItem.name ||!newItem.price ||!newItem.categoryId){
      alert("Ime, cijena, kategorija obavezni"); return;
    }
    setSaving("new");
    ...
    if(!r.ok){ alert(j.error); }
      ...
      setNewItem({ name:"", price:"", categoryId:cats[0]?.id||"", description:"", imageUrl:"", isBoosted:false, boostLevel:0, available:true });
    }
    setSaving(null);
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-6">Dodaj artikal sa slikom, cijenom i boostom. Boostani se prikazuju na vrhu gostu.</p>
      ...
      <div className="border rounded-xl p-4 bg-gray-50 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
          ...
          <div className="flex flex-col gap-2">
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={newItem.isBoosted} onChange={e=>setNewItem({...newItem, isBoosted:e.target.checked })}/> 🔥 Boost (prikaz na vrhu)</label>
            {newItem.isBoosted && (
              <input type="number" placeholder="Boost level 0-100" value={newItem.boostLevel} onChange={e=>setNewItem({...newItem, boostLevel:Number(e.target.value) })} className="border rounded px-3 py-1 w-40"/>
            )}
          </div>
        </div>
      ...
      <div className="flex gap-3 mb-4">
        ...
        <select value={catFilter} onChange={e=>setCatFilter(e.target.value)} className="border rounded px-3 py-2">
        ...
        {filtered.map((it)=>(
          <div key={it.id} className={`border rounded-xl p-3 bg-white flex gap-3 ${!it.available?"opacity-50":""}`}>
            <div className="flex-1 grid grid-cols-1 md:grid-cols-5 gap-2">
              ...
              <select value={it.categoryId} onChange={e=>setItems(p=>p.map(x=>x.id===it.id?{...x,categoryId:e.target.value}:x))} className="border rounded px-2 py-1">
                {cats.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
              ...
              <div className="flex gap-2 items-center">
                ...
                {it.isBoosted && <input type="number" value={it.boostLevel} onChange={e=>setItems(p=>p.map(x=>x.id===it.id?{...x,boostLevel:Number(e.target.value)}:x))} className="w-12 border rounded px-1 text-xs"/>}
              </div>
            </div>
