  ...
  return (
    <>
      ...
      <aside className={`fixed lg:sticky top-0 z-50 h-screen w-64 bg-[#0F172A] text-slate-300 flex flex-col border-r border-slate-800 shrink-0 transition-transform duration-300 lg:translate-x-0 ${mobileOpen? "translate-x-0" : "-translate-x-full"}`}>
        <div className="h-16 flex items-center gap-3 px-6 border-b border-slate-800/80 shrink-0">
          ...
          <div className="flex-1">
            ...
            <div className="text-xs text-slate-400 -mt-0.5 truncate max-w-">{restaurant?.name || "Admin panel"}</div>
          </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-5 space-y-6">
          ...
          {navGroups.map(group => (
            <div key={group.label}>
              ...
              <div className={`mt-1 space-y-1 overflow-hidden transition-all ${openGroups[group.label]? "max-h- opacity-100" : "max-h-0 opacity-0"}`}>
                {group.items.map(item => {
                  const active = isActive(item.href, (item as any).exact)
                  return (
                    <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm relative ${active? "bg-white text-slate-900 shadow-lg" : "hover:bg-slate-800/80 hover:text-white text-slate-400"}`}>
                      ...
                      <div className="flex-1">
                        ...
                        <div className={`text-xs mt-0.5 ${active? "text-slate-500" : "text-slate-500"}`}>{(item as any).desc}</div>
                      </div>
