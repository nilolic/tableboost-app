  ...
  return (
    <main className="max-w-6xl mx-auto p-8">
      <div className="flex justify-between items-start mb-6">
        <div>{data.impersonate&&<div className="bg-yellow-500 text-black px-4 py-2 rounded-full text-sm mb-4 inline-block">Super Admin mod - gledaš kao {r.name} <a href="/api/superadmin/impersonate?clear=1" className="underline ml-2 font-bold">Izađi</a></div>}<h1 className="text-4xl font-black mb-2">{r.name}</h1><p className="text-neutral-600">Slug: {r.slug} • Stolova: {r.tables.length} • Korisnika: {r.users.length}</p></div>
        <button onClick={logout} className="border border-black px-5 py-2.5 rounded-full text-sm">Odjava</button>
      </div>
      <div className="grid md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white border rounded-2xl p-6"><div className="text-sm text-neutral-500">Konobari</div><div className="text-3xl font-bold mt-2">{r.users.filter((u:any)=>u.role==='WAITER').length}</div></div>
        <div className="bg-white border rounded-2xl p-6"><div className="text-sm text-neutral-500">Kuhinja</div><div className="text-3xl font-bold mt-2">{r.users.filter((u:any)=>u.role==='KITCHEN').length}</div></div>
        <div className="bg-white border rounded-2xl p-6"><div className="text-sm text-neutral-500">Artikala</div><div className="text-3xl font-bold mt-2">{r._count.items}</div></div>
      </div>
