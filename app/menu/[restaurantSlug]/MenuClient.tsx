            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0 overscroll-contain">
              ...
              {cartDetailed.length>0 && (<>
                    ...
                    {cartDetailed.map((i:any)=>(
                      <div key={i.id} className="flex gap-3 border-2 border-zinc-900 p-3 rounded-2xl bg-white shadow-sm">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between"><span className="font-black text-">{t(i.name,i.nameEn,i.nameDe)}</span><span className="font-black text-">{(i.price*i.qty).toFixed(2)}E</span></div>
                  ...
                  <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-3">
                    <div className="text- font-black uppercase tracking-wider text-amber-700 mb-1">Napomena za kuhinju / alergije - cijela narudzba</div>
                  ...
                  <div><div className="text- font-black uppercase tracking-wider opacity-60 mb-2 flex justify-between"><span>Tip</span>{tipPercent>0 && <span className="text-black">+{tipAmount.toFixed(2)}E</span>}</div><div className="grid grid-cols-4 gap-2">{[0,10,20,30].map(pct=>(<button key={pct} onClick={()=>setTipPercent(pct)} className={`h-10 rounded-full font-bold text- border-2 ${tipPercent===pct? 'bg-black text-white border-black' : 'bg-white border-zinc-200'}`}>{pct===0? 'Bez tipa' : `${pct}%`}</button>))}</div></div>
                  <div className="text- font-black uppercase tracking-wider opacity-60">Nacin placanja</div>
                  <div className="grid grid-cols-3 gap-2">
                    {payCashEnabled && <button onClick={()=>setPaymentMethod('CASH')} className={`p-3 rounded-2xl border-2 text-left ${paymentMethod==='CASH'?'border-black bg-black text-white':'border-zinc-200 bg-white'}`}><div>$</div><div className="font-bold text- mt-1">Gotovina</div></button>}
                    {payTerminalEnabled && <button onClick={()=>setPaymentMethod('CARD_TERMINAL')} className={`p-3 rounded-2xl border-2 text-left ${paymentMethod==='CARD_TERMINAL'?'border-black bg-black text-white':'border-zinc-200 bg-white'}`}><div>Card</div><div className="font-bold text- mt-1">POS</div></button>}
                    {payOnlineEnabled? <button onClick={()=>setPaymentMethod('CARD_ONLINE')} className={`p-3 rounded-2xl border-2 text-left ${paymentMethod==='CARD_ONLINE'?'border-black bg-black text-white':'border-zinc-200 bg-white'}`}><div>Web</div><div className="font-bold text- mt-1">Online</div></button> : <button disabled className="p-3 rounded-2xl border-2 border-zinc-100 bg-zinc-100 opacity-50 text-left"><div>Web</div><div className="font-bold text- mt-1">Online</div></button>}
                  </div>
                  <div className="bg-white border rounded-2xl p-3 space-y-1 text-"><div className="flex justify-between"><span className="text-zinc-500">{T.subtotal}</span><span className="font-bold">{subtotal.toFixed(2)}E</span></div>{tipPercent>0 && <div className="flex justify-between"><span className="text-zinc-500">{T.tip} {tipPercent}%</span><span className="font-bold">+{tipAmount.toFixed(2)}E</span></div>}<div className="flex justify-between font-black text- pt-1 border-t mt-1"><span>{T.total}</span><span>{total.toFixed(2)}E</span></div></div>
                  {upsells.length>0 && (<div className="bg-white border-2 border-dashed border-amber-200 rounded-2xl p-3"><div className="text- font-black uppercase tracking-widest mb-2">Preporucujemo</div><div className="space-y-2">{upsells.slice(0,3).map((u:any)=>(<div key={u.id} className="flex gap-2 items-center bg-amber-50 border border-amber-200 rounded-2xl p-2.5"><div className="w-12 h-12 rounded-xl bg-white overflow-hidden shrink-0"><img src={u.imageUrl||""} className="w-full h-full object-cover"/></div><div className="flex-1 min-w-0"><div className="font-bold text- line-clamp-1">{t(u.name,u.nameEn,u.nameDe)}</div><div className="text- text-zinc-500">{u.price.toFixed(2)}E</div></div><button onClick={()=>add(u.id)} className="bg-black text-white rounded-full px-3 h-8 text- font-black">+ Dodaj</button></div>))}</div></div>)}
                </>)}
            </div>
            {cartDetailed.length>0 && (<div className="p-4 border-t bg-white shrink-0"><button disabled={sending} onClick={order} className="w-full h-14 bg-black text-white rounded-full font-black text- disabled:opacity-50 flex items-center justify-center gap-2 shadow-xl shadow-black/20 active:scale-[0.98] transition">{sending? 'Saljem...' : `${T.order} • ${total.toFixed(2)}E`}</button></div>)}
          </div>
        </div>
      )}
 </div>
