...
export async function POST(req: Request){
  ...
  try{
    const { name, slug, ownerName, ownerEmail, ownerPass, soloMode } = await req.json()
    ...
    const r = await prisma.restaurant.create({ data: { name, slug, soloMode:!!soloMode, users:{ create:{ email:ownerEmail, name:ownerName, password:hash, role:'RESTAURANT_ADMIN' } }, tables:{ create: Array.from({length:10},(_,i)=>({ number:i+1, qrSlug: crypto.randomBytes(6).toString('base64url') })) } } })
