...
export async function POST(req:Request){
  ...
  try{
    ...
    const restaurant = await prisma.restaurant.create({
      data: {
        name, slug, legalName: name, city: 'Split', description: 'DEMO restoran - 14 dana',
        isDemo: true, expiresAt,
        users: { create: [
    ...
    return NextResponse.json({ok:true, restaurant, creds: { admin:`${slug}-admin@demo.local / Sef12345!`, link:`https://tableboost.app/login`, from: now, to: expiresAt, days: 14 }})
